...
void add_entry(message * response, message *receiveMessagePtr){
    ...
    database[recordCount].timeStamp = seconds();
...
Boolean isNeighbour(char node) {
	...
	diag("Node# %d is not a neighbour, use find to find neighbour if it exists", node);
...
// - - - - - - Assemble Common Parts and Send Message - - - - - - - //
    state Transmit_Message:
        ...
        call transmitter(messagePtr, Transmit_Message_Delay);
    state Transmit_Message_Delay:
        delay(3*SECOND, Transmit_Message_Result);

        release;

	state Transmit_Message_Result:
		if(currentRequestNumber != 0) {
			diag("Failed to reach the destination");
			currentRequestNumber = 0;
		}
